...
UINT8 CModel3::ReadInputs(unsigned reg)
{
	...
	case 0x04:	// current input bank
		...
		if ((inputBank&1) == 0)
		{
			data &= ~(Inputs->coin[0]<<0);		// Coin 1
			data &= ~(Inputs->coin[1]<<1);		// Coin 2
			data &= ~(Inputs->test[0]<<2);		// Test A
			data &= ~(Inputs->service[0]<<3);	// Service A
			data &= ~(Inputs->start[0]<<4);		// Start 1
			data &= ~(Inputs->start[1]<<5);		// Start 2
		}
		else
		{
			data &= ~(Inputs->service[1]<<6);	// Service B
			data &= ~(Inputs->test[1]<<7);		// Test B
	...
	case 0x08:	// game-specific inputs
		...
		{
			data &= ~(Inputs->up[0]<<5);		// 1 Up
			data &= ~(Inputs->down[0]<<4);		// 1 Down
			data &= ~(Inputs->left[0]<<7);		// 1 Left
			data &= ~(Inputs->right[0]<<6);		// 1 Right
		}

		if ((Game->inputFlags&GAME_INPUT_FIGHTING))
		{
			data &= ~(Inputs->escape[0]<<3);	// 1 Escape
			data &= ~(Inputs->guard[0]<<2);		// 1 Guard
			data &= ~(Inputs->kick[0]<<1);		// 1 Kick
			data &= ~(Inputs->punch[0]<<0);		// 1 Punch
		}
		
		if ((Game->inputFlags&GAME_INPUT_SOCCER))
		{
			data &= ~(Inputs->shortPass<<2);	// 1 Short Pass
			data &= ~(Inputs->longPass<<0);		// 1 Long Pass
			data &= ~(Inputs->shoot<<1);		// 1 Shoot
		}

		if ((Game->inputFlags&GAME_INPUT_VR))
		{
			data &= ~(Inputs->vr[0]<<0);		// VR1 Red
			data &= ~(Inputs->vr[1]<<1);		// VR2 Blue
			data &= ~(Inputs->vr[2]<<2);		// VR3 Yellow
			data &= ~(Inputs->vr[3]<<3);		// VR4 Green
		}
		
		if ((Game->inputFlags&GAME_INPUT_SHIFT4))
		{
			if (Inputs->gearShift4[1])			// Shift 2
				data &= ~0x60;
			else if (Inputs->gearShift4[3])		// Shift 4
				data &= ~0x20;
			if (Inputs->gearShift4[0])			// Shift 1
				data &= ~0x50;
			else if (Inputs->gearShift4[2])		// Shift 3
				data &= ~0x10;
		}
		
		if ((Game->inputFlags&GAME_INPUT_RALLY))
		{
			data &= ~(Inputs->viewChange<<0);	// View change
			data &= ~(Inputs->handBrake<<1);	// Hand brake
		}
		
		if ((Game->inputFlags&GAME_INPUT_GUN1))
			data &= ~(Inputs->trigger<<0);		// Trigger
		
		if ((Game->inputFlags&GAME_INPUT_ANALOG_JOYSTICK))
		{
			data &= ~(Inputs->analogJoyTrigger<<5);	// Trigger
			data &= ~(Inputs->analogJoyEvent<<0);	// Event Button
		}
		
		if ((Game->inputFlags&GAME_INPUT_TWIN_JOYSTICKS))	// First joystick
		{
			/*
			 ...
			 * Reverse: 	1D 2D
			 */
			if (Inputs->twinJoyTurnLeft)
				data &= ~0x10;
			else if (Inputs->twinJoyTurnRight)
				data &= ~0x20;
			else if (Inputs->twinJoyForward)
				data &= ~0x20;
			else if (Inputs->twinJoyReverse)
				data &= ~0x10;
			 ...
			 * Crouch: 			1R 2L
			 */
			if (Inputs->twinJoyStrafeLeft)
				data &= ~0x80;
			else if (Inputs->twinJoyStrafeRight)
				data &= ~0x40;
			else if (Inputs->twinJoyJump)
				data &= ~0x80;
			else if (Inputs->twinJoyCrouch)
				data &= ~0x40;
				
			// Shot trigger and Turbo
			data &= ~(Inputs->twinJoyLeftShot<<0);
			data &= ~(Inputs->twinJoyLeftTurbo<<1);
		}
	...
	case 0x0C:	// game-specific inputs
		...
		if ((Game->inputFlags&GAME_INPUT_TWIN_JOYSTICKS))	// Second joystick (see register 0x08 for comments)
		{
			if (Inputs->twinJoyTurnLeft)
				data &= ~0x20;
			else if (Inputs->twinJoyTurnRight)
			...
			data &= ~(Inputs->twinJoyRightShot<<0);
			data &= ~(Inputs->twinJoyRightTurbo<<1);
		}
	...
	case 0x3C:	// ADC
		...
		{
			adc[0] = Inputs->steering;
			adc[1] = Inputs->accelerator;
			adc[2] = Inputs->brake;
		}
		if ((Game->inputFlags&GAME_INPUT_ANALOG_JOYSTICK))
		{
			adc[0] = Inputs->analogJoyY;
			adc[1] = Inputs->analogJoyX;
		}
		...
		case 0x87:		// Read light gun register
			...
			if ((Game->inputFlags&GAME_INPUT_GUN1))
			{
				...
				case 0:	// Player 1 gun Y (low 8 bits)
					serialFIFO2 = Inputs->gunY&0xFF;
					break;
				case 1:	// Player 1 gun Y (high 2 bits)
					serialFIFO2 = (Inputs->gunY>>8)&3;
					break;
				case 2:	// Player 1 gun X (low 8 bits)
					serialFIFO2 = Inputs->gunX&0xFF;
					break;
				case 3:	// Player 1 gun X (high 2 bits)
					serialFIFO2 = (Inputs->gunX>>8)&3;
					break;
				...
				case 8:	// Off-screen indicator (bit 0 = player 1, bit 1 = player 2, set indicates off screen)
					serialFIFO2 = 2|Inputs->offscreen;
					break;
				default:
...
static void Dump(const char *file, UINT8 *buf, unsigned size, BOOL reverse)
{
	FILE	*fp;
...
#define MEMORY_POOL_SIZE	(0x800000+0x800000+0x8000000+0x4000000+0x20000+0x20000)
...
void CModel3::AttachInputs(CModel3Inputs *InputsPtr)
{
	Inputs = InputsPtr;
...
#if 0
	Dump("ram", ram, 0x800000, TRUE);
	Dump("vrom", vrom, 0x4000000, TRUE);
	//Dump("crom", crom, 0x800000, TRUE);
	//Dump("bankedCrom", &crom[0x800000], 0x7000000, TRUE);
#endif
