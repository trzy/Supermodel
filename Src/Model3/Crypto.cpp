 ...
 * Crypto.cpp
 *
 * Security board encryption device emulation. This code was taken from MAME
 * (http://mamedev.org).
 */
...
void CCrypto::SaveState(CBlockFile *SaveState)
{
  if (!key)
    return; // no security board
  SaveState->NewBlock("Sega 315-5881", __FILE__);	
	SaveState->Write(buffer.get(), BUFFER_SIZE);
	SaveState->Write(line_buffer.get(), LINE_SIZE);
	SaveState->Write(line_buffer_prev.get(), LINE_SIZE);
	SaveState->Write(&prot_cur_address, sizeof(prot_cur_address));
	SaveState->Write(&subkey, sizeof(subkey));
	SaveState->Write(&enc_ready, sizeof(enc_ready));
	SaveState->Write(&dec_hist, sizeof(dec_hist));
	SaveState->Write(&dec_header, sizeof(dec_header));
	SaveState->Write(&buffer_pos, sizeof(buffer_pos));
	SaveState->Write(&line_buffer_pos, sizeof(line_buffer_pos));
	SaveState->Write(&line_buffer_size, sizeof(line_buffer_size));
}

void CCrypto::LoadState(CBlockFile *SaveState)
{
  if (!key)
    return;
  ...
  if (SaveState->FindBlock("Sega 315-5881") != Result::OKAY)
	...
	SaveState->Read(buffer.get(), BUFFER_SIZE);
	SaveState->Read(line_buffer.get(), LINE_SIZE);
	SaveState->Read(line_buffer_prev.get(), LINE_SIZE);
	SaveState->Read(&prot_cur_address, sizeof(prot_cur_address));
	SaveState->Read(&subkey, sizeof(subkey));
	SaveState->Read(&enc_ready, sizeof(enc_ready));
	SaveState->Read(&dec_hist, sizeof(dec_hist));
	SaveState->Read(&dec_header, sizeof(dec_header));
	SaveState->Read(&buffer_pos, sizeof(buffer_pos));
	SaveState->Read(&line_buffer_pos, sizeof(line_buffer_pos));
	SaveState->Read(&line_buffer_size, sizeof(line_buffer_size));
}
...
UINT16 CCrypto::Decrypt(UINT8 **base)
{
  if (key == 0)
    return 0xFFFF;
	if(!enc_ready)
...
void CCrypto::SetAddressLow(UINT16 data)
{
	prot_cur_address = (prot_cur_address & 0xffff0000) | data;
	enc_ready = false;
}

void CCrypto::SetAddressHigh(UINT16 data)
{
...
void CCrypto::SetSubKey(UINT16 data)
{
	subkey = data;
	enc_ready = false;
}
...
const CCrypto::sbox CCrypto::fn1_sboxes[4][4] = {
	{   // 1st round
		{
			{
...
const CCrypto::sbox CCrypto::fn2_sboxes[4][4] = {
	{   // 1st round
		{
			{
...
const int CCrypto::fn1_game_key_scheduling[FN1GK][2] = {
...
const int CCrypto::fn2_game_key_scheduling[FN2GK][2] = {
...
const int CCrypto::fn1_sequence_key_scheduling[20][2] = {
	{0,52},  {1,34},  {2,17},  {3,36}, {4,84},  {4,88},  {5,57},  {6,48},
	{6,68},  {7,76},  {8,83},  {9,30}, {10,22}, {10,41}, {11,38}, {12,55},
	{13,74}, {14,19}, {14,80}, {15,26}
};

const int CCrypto::fn2_sequence_key_scheduling[16] = {77,34,8,42,36,27,69,66,13,9,79,31,49,7,24,64};

const int CCrypto::fn2_middle_result_scheduling[16] = {1,10,44,68,74,78,81,95,2,4,30,40,41,51,53,58};

int CCrypto::feistel_function(int input, const struct sbox *sboxes, UINT32 subkeys)
{
	int k,m;
	int aux;
	int result=0;
...
UINT16 CCrypto::block_decrypt(UINT32 game_key, UINT16 sequence_key, UINT16 counter, UINT16 data)
{
	int j;
	int aux, aux2;
	int A, B;
...
UINT16 CCrypto::get_decrypted_16()
{
	UINT16 enc;
...
11111111 - empty node
*/
const UINT8 CCrypto::trees[9][2][32] = {
	{
...
int CCrypto::get_compressed_bit()
{
//  if(buffer_pos == BUFFER_SIZE)
//      enc_fill();
