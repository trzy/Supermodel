...
ssh %VM_USER%@%VM_HOST% "bash -lc 'set -e; rm -rf \"%REMOTE_WORKDIR%\"; mkdir -p \"%REMOTE_WORKDIR%\" \"%REMOTE_CORE_DIR%\" \"%REMOTE_INFO_DIR%\" \"%REMOTE_SYSTEM_DIR%\" \"%REMOTE_SAVE_DIR%\" \"%REMOTE_TEST_DIR%\"; tar -xf /tmp/Supermodel-linux-stage.tar -C \"%REMOTE_WORKDIR%\"; rm -f /tmp/Supermodel-linux-stage.tar; cd \"%REMOTE_WORKDIR%\"; make -f Makefiles/Makefile.UNIX LIBRETRO=1 BITS=64 -j2; cp \"lib64/supermodel_libretro.so\" \"%REMOTE_CORE_DIR%/supermodel_libretro.so\"'"
if errorlevel 1 exit /b 1
